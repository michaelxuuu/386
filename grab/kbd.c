...
// https://wiki.osdev.org/%228042%22_PS/2_Controller More on ps/2 keyboard,
...
key_event_t handle_sequence_state(uint8_t scancode, const uint8_t *sequence,
                                  uint16_t length, parser_state_t next_state,
                                  int is_break, int key_code);
...
uint8_t is_make_code(uint8_t scancode) { return !(scancode & 0x80); }

// Convert break code to make code
uint8_t to_make_code(uint8_t breakcode) { return breakcode & 0x7F; }
...
key_event_t null_event() { return TO_INT((struct key_event){0}); }

// Create an data event, returned by the parser when it has successfully
// completed parsing a scan code, this scan code satisfies none of three the
// "NULL" properties listed above, no errors (unrecognized scan codes) have
// occurred during parsing
key_event_t data_event(int data, int is_keypad)
{
        struct key_event e = {.hasdata = 1,
                              .data = data,
                              .keypad = is_keypad,
                              .kbdstate = kbd.state};
        return TO_INT(e);
...
void update_state(int key, int is_make)
{
        switch (key) {
        case KEY_L_CTRL:
                kbd.state.lctrl = is_make;
                break;
        case KEY_R_CTRL:
                kbd.state.rctrl = is_make;
                break;
        case KEY_L_SHFT:
                kbd.state.lshft = is_make;
                break;
        case KEY_R_SHFT:
                kbd.state.rshft = is_make;
                break;
        case KEY_L_ALT:
                kbd.state.lalt = is_make;
                break;
        case KEY_R_ALT:
                kbd.state.ralt = is_make;
                break;
        case KEY_NUM:
                kbd.state.numlock ^= is_make;
                break;
        case KEY_CAPS:
                kbd.state.capslock ^= is_make;
                break;
        }
}

// The main scan code parsing DFA:
// Each call to this function consumes 1 scan byte instead of 1 complete scan
// code, starting from the current driver index. Returns a non-NULL key_event if
// the scanbyte can be interrpreted as an data character; and a NULL event is
// reported back to the user routine in all other case, including:
// 1. It is a state-related key (such as ctrl and caplock), which only change
// the keyboard internal states
// 2. It is a break key (key release), which in most cases are ignored, and
// which, in some cases, also requires chhanging some internal states (such as
// ctrl and shift) of the keyboard, so that they won't be included in the later
// data events (events with data data) reported to the user
...
key_event_t parse_scancode(uint8_t scancode)
{
        ...
        case STATE_PAUSE:
                return handle_sequence_state(scancode, pause_sequence,
                                             sizeof(pause_sequence),
                                             STATE_PAUSE, 0, KEY_PAUSE);
        case STATE_EXTENDED:
                return handle_extended_state(scancode);
        case STATE_PRINT_MAKE:
                return handle_sequence_state(scancode, print_make_sequence,
                                             sizeof(print_make_sequence),
                                             STATE_PRINT_MAKE, 0, KEY_PRNT);
        case STATE_PRINT_BREAK:
                return handle_sequence_state(scancode, print_break_sequence,
                                             sizeof(print_break_sequence),
                                             STATE_PRINT_BREAK, 1, KEY_PRNT);
        default:
                return null_event();
        }
}
...
key_event_t handle_init_state(uint8_t scancode)
{
        ...
        if (make_code > MAX_SCANCODE) return raw_event(scancode);
...
key_event_t handle_extended_state(uint8_t scancode)
{
        ...
        case 0x35:
                return data_event('\n', 1);
        case 0x48:
                return data_event(KEY_U_ARROW, 0);
        case 0x4b:
                return data_event(KEY_L_ARROW, 0);
        case 0x4d:
                return data_event(KEY_R_ARROW, 0);
        case 0x50:
                return data_event(KEY_D_ARROW, 0);
        case 0x53:
                return data_event(KEY_DELETE, 0);
        case 0x1d:
                update_state(KEY_R_CTRL, 1);
                break;
        case 0x9d:
                update_state(KEY_R_CTRL, 0);
                break;
        case 0x38:
                update_state(KEY_R_ALT, 1);
                break;
        case 0xb8:
                update_state(KEY_R_ALT, 0);
                break;
        case 0x2a:
                kbd.parser_state = STATE_PRINT_MAKE;
                break;
        case 0xb7:
                kbd.parser_state = STATE_PRINT_BREAK;
                break;
        default:
...
{
        if (!TO_STRUCT(event).kbdstate.lshft &&
            !TO_STRUCT(event).kbdstate.rshft)
                return c;
