...
static int getXY(char *s, int *X, int *Y, char *caller)
{
        ...
        if (strlen(s) < 7 || s[0] != '(' || s[1] != 'h' || s[2] != 'd' ||
            !isdigit(s[3]) || s[4] != ',' || !isdigit(s[5]) || s[6] != ')') {
        ...
        if (fs_init(&partitions[y], ide_read_lba, ide_write_lba,
                    (printfunc)printf) < 0) {
...
static void set(char *args)
{
        ...
        if (!strncmp("root", name, 4)) {
                ...
                if (getXY(value, &X, &Y, "set") < 0) return;
...
// The word consumed is returned by 'word.' The word returned is already
// null-terminated. Return a pointer to the next word or 0 if there're no more
// words.
char *nextword(char *p, char **word)
{
        for (; *p && isspace(*p); p++)
