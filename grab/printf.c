...
void printf_set_color(uint8_t c) { color = c; }
...
static void putchar(char c)
{
        ...
        case '\r':
                ...
                break;
        case '\b':
                if (off != 0) vga_putchar(--off, ' ', color);
                break;
        case '\t':
                off += 4;
                break;
...
void printf(char *s, ...)
{
        ...
        while (*s) {
                ...
                int n; // Number of characters printed by the type specifier
                       // handling functions - _char(), _string(), and _int()
                       // Used to determine how many space left in width for
                       // padding spaces from behind Note. We do not support
                       // front padding!
                switch (state) {
                case REGULAR:
                        if (*s == '%')
