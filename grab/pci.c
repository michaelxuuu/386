...
//                          | PCI device 2
//                          | ...
//                          | PCI device 31 [why 31? look at the device number
//                          of the address format below]
//
// PCI controller is assigned 2 i/o ports - 0xcf8 for its address register and
// 0xcfc for its data register. CPU communicates with it through accessing these
// two i/o ports using in/out instructions. Using only two ports *is* sufficient
// for the CPU to program the PCI devices  - CPU first writes to the address
// register to specify which device it wants to talk to and depending on whether
// it's a read or write operation, it goes on read or write to the data
// register.
//
//
...
// -----------+----------+------------+--------------+---------------+------------------
// Enable Bit |Reserved	 |Bus Number  |Device Number |Function Number|Register
// Offset where the register offset has to be dword aligned and thus its bits
// 1:0 are always 0
//
//
...
//
// Register	| Offset    |	Bits 31-24	| Bits 23-16    | Bits 15-8
// | Bits 7-0
// ---------+-----------+---------------+---------------+---------------+-------------
// 0x0	    | 0x0	    |   Device  ID	                | Vendor ID
// 0x1	    | 0x4	    |   Status                      | Command
// 0x2	    | 0x8	    |   Class code	| Subclass	    | Prog IF
// | Revision ID 0x3	    | 0xC	    |   BIST        | Header type
// | Latency Timer | Cache Line Size
//
// Header Type identifies the layout of the rest of the header beginning at byte
// 0x10 of the header. If bit 7 of this register is set, the device has multiple
...
static void _pci_write_dword(int bus, int dev, int func, int dwoff,
                             uint32_t data)
{
        outl(mkaddr(1, bus, dev, func, (dwoff << 2)), PORT_ADDR);
        outl(data, PORT_DATA);
...
// This scan is recursive cus PCI devices can from a tree structure and have
// secondary buses as in the case of having a PCI to PCI bridge.
//
void _pci_prob_dev(uint8_t bus)
{
        for (int dev = 0; dev < NDEV_PER_BUS; dev++) {
                ...
                for (int fun = 0; fun < nfun; fun++) {
                        ...
                        // and if it is we need to retrieve the secondary PCI
                        // bus ID from it. This is where the optimization comes
                        // in - you scan as many bus as there are rather than
                        // "brutal force" scan over all 256 possible buses!
                        ...
                        if (p->hdr.class == PCI_CLASS_BRIDGE &&
                            p->hdr.subclass == PCI_SUBCLASS_PCI2PCI) {
                                // PCI to PCI bridge function detected. Start
                                // recursion!
                                uint8_t secondarybus =
                                    _pci_read_byte(bus, dev, fun, 25);
...
void pci_prob_dev() { _pci_prob_dev(0); }
...
pcidev_t pci_get_dev(uint16_t vendorid, uint16_t deviceid)
{
        for (int i = 0; i < npcidev; i++) {
                ...
                if (vendorid != p->hdr.vendorid) continue;
                if (deviceid != p->hdr.deviceid) continue;
                return i;
        }
        return -1;
}
...
void pci_list()
{
        ...
        for (int i = 0; i < npcidev; i++) {
                ...
                printf("%8d%8d%8d%8x%8x%8x%8x\n", dev->bus, dev->dev, dev->fun,
                       dev->hdr.vendorid, dev->hdr.deviceid, dev->hdr.class,
                       dev->hdr.subclass);
        }
}
